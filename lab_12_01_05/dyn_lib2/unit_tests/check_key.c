...
START_TEST(pos_01)
{
    ...
    const int *pekey = expected + n;
    pb2 = malloc(sizeof(int) * n);
    if (!pb2)
        ck_assert_int_eq(1, 0);
    pe2 = pb2 + n;

    int rc = key(pb1, pe1, &pb2, &pe2, (size_t)n);
...
START_TEST(neg_01)
{
    ...
    int rc = key(pb1, pe1, &pb2, &pe2, (size_t)make_pre_key(pb1, pe1));
...
START_TEST(neg_02)
{
    ...
    int rc = key(pe1, pb1, &pb2, &pe2, (size_t)make_pre_key(pb1, pe1));
